      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 pb-24 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-1">
            <h1 className="text-2xl md:text-3xl font-extrabold text-slate-900 tracking-tight">Drive Applicants</h1>
            <p className="text-slate-500 text-sm font-medium">{filteredApps.length} candidates found for this drive</p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
            <div className="relative w-full sm:flex-1 md:w-64">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-2.5 rounded-xl border border-slate-200 bg-white focus:outline-none focus:ring-4 focus:ring-indigo-100 focus:border-indigo-500 transition-all shadow-sm text-sm font-medium placeholder:text-slate-400"
                value={search}
            ...
            <div className="relative w-full sm:w-48">
              ...
              <select
                className="w-full pl-11 pr-10 py-2.5 rounded-xl border border-slate-200 bg-white focus:outline-none focus:ring-4 focus:ring-indigo-100 focus:border-indigo-500 shadow-sm appearance-none text-sm font-semibold text-slate-700 cursor-pointer"
                value={statusFilter}
                  ...
                  {filteredApps.map((app) => (
                    <tr key={app._id} className="hover:bg-indigo-50/30 transition-colors group">
                      ...
                      <td className="px-6 py-4 text-right flex items-center justify-end gap-3 pr-8">
                        {/* Quick Actions */}
                        <div className="flex gap-1 md:opacity-0 group-hover:opacity-100 transition-all duration-300 md:translate-x-2 group-hover:translate-x-0">
                          <button
                            onClick={() => updateStatus(app._id, "Shortlisted")}
                            title="Shortlist"
                            className="p-2 hover:bg-amber-50 text-amber-500 hover:text-amber-600 rounded-xl transition-colors bg-white border border-slate-100 shadow-sm"
                          >
                            <UserCheck size={18} />
                          </button>
                          <button
                            ...
                            title="Select"
                            className="p-2 hover:bg-emerald-50 text-emerald-500 hover:text-emerald-600 rounded-xl transition-colors bg-white border border-slate-100 shadow-sm"
                          >
                            <CheckCircle size={18} />
                          </button>
                          <button
                            ...
                            title="Reject"
                            className="p-2 hover:bg-red-50 text-red-500 hover:text-red-600 rounded-xl transition-colors bg-white border border-slate-100 shadow-sm"
                          >
                            <XCircle size={18} />
                          </button>
                        </div>
